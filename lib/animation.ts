...
/** Ball ends in goal when the final keyframe is inside either post. */
 ...
 * - whether the ball ends in a goal (for celebrate)
 */
export function getAnimState(
  ...
  if (currentTime > duration - 1200 && duration > 2000 && ballEndsInGoal(ball)) {
    return 'celebrate';
  }
  ...
  if ((ballEvent === 'shot' || ballEvent === 'pass' || ballEvent === 'cross')
      && distToBall < 0.1)                                                                    return 'kick';
